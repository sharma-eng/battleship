import { SHIP_SPECS, type Board, type ShipPlacement, type ShipId, type Orientation } from './types.js';
...
export function processShot(
  ...
  if (row < 0 || row >= GRID_SIZE || col < 0 || col >= GRID_SIZE) return { hit: false };
  const cell = board[row][col];
  if (cell.state === 'hit' || cell.state === 'miss' || cell.state === 'sunk') return { hit: false };
  if (cell.state === 'empty') {
  ...
  if (sunk) {
    ...
    return { hit: true, sunkShipId: shipId, sunkShipName: name, allSunk: allShipsSunk(board) };
  }
  return { hit: true };
}
...
function markShipSunk(board: Board, shipId: ShipId): void {
  for (let r = 0; r < GRID_SIZE; r++) {
    for (let c = 0; c < GRID_SIZE; c++) {
      if (board[r][c].shipId === shipId) board[r][c] = { ...board[r][c], state: 'sunk' };
    }
  }
}
